...
public class CollectListFactory {
	...
	public static Bundle parseResult(String wsResponse) throws JSONException {
		...
		if (jsonObject.has(ResponseBean.RESPONSE_LIST)) {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				if (item.has(ResponseBean.RESPONSE_SHOP_PRICE)) {
					map.put(ResponseBean.RESPONSE_SHOP_PRICE, item.getString(ResponseBean.RESPONSE_SHOP_PRICE));
				}
