...
public class Result {
	 ...
	 * 
	 * @param result
	 *            the String you should handle to get the money info.
	 * @return the money you paid through aliPay.
	 */
	// TODO added by wang.
	private String getMoney(String result) {
		String totalFee = null;
		try {
			JSONObject json = string2JSON(result, "&");

			int pos = result.indexOf("&total_fee=");
			String signContent = result.substring(0, pos);

			totalFee = json.getString("total_fee");
		...
		Log.i("Result", "total_fee =" + totalFee);
		return totalFee;
	}
