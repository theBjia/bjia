...
/**发送消息工具类
 * emit(String name, JSONArray args)*/
...
public class SocketIOClient {
	...
	private static byte[] readToEndAsArray(InputStream input)
			throws IOException {
	...
	public void emit(String name, JSONArray args, final Acknowledge acknowledge)
	...
	public void emit(String name, JSONArray args,
			final Acknowledge acknowledge, final String endpoint)
			throws JSONException {
		...
		if (mSendHandler != null) {
			mSendHandler.post(new Runnable() {
				...
				public void run() {
					...
					// mClient.send(String.format("5:" + nextId + (acknowledge
					// ==
					// null ? "" : "+") + ":%s:%s", (endpoint == null ? "" :
					// endpoint), event.toString()));
	...
	private void cleanup() {
		if (mClient != null)
			mClient.disconnect();
		mClient = null;
		if (mMessageIdCounter != null)
			mMessageIdCounter.set(0);
		if (mAcknowledges != null)
			mAcknowledges.clear();
		if (mSendLooper != null) {
			mSendLooper.quit();
			mSendLooper = null;
