...
public class ZitidianfenbuFragment extends BaseFragment implements
	...
	private void addMarkersToMap() {
		for (int i = 0; i < arrayList.size(); i++) {
			...
			doulat = StringUtil.strToDouble(map.get("lat") + "");
			doulng = StringUtil.strToDouble(map.get("lng") + "");
	...
	public void addMakerToMap(double lat, double lng) {
		MarkerOptions markerOption = new MarkerOptions();
			...
			if (mRequestId == requestId) {
				...
				if (result == 1) {
					...
					if (!StringUtil.isEmpty(MyApplication.geoLat)) {
						addMakerToMap(
								StringUtil.strToDouble(MyApplication.geoLat),
								StringUtil.strToDouble(MyApplication.geoLng));
						CameraPosition cameraPosition = new CameraPosition(
								new LatLng(
										StringUtil
												.strToDouble(MyApplication.geoLat),
										StringUtil
												.strToDouble(MyApplication.geoLng)),
								13, 0, 0);
