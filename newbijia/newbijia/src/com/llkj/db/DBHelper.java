...
public class DBHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL(getCreateOtherChatSql());
	...
	private String getCreateNoticeContactsSql() {
		...
		buff.append("CREATE TABLE if not exists ")
				...
				.append("userId TEXT,").append("backgroundPic TEXT,")
				.append("isMember TEXT)");
		return buff.toString();
	}
	...
	private String getCreateOtherChatSql() {
		StringBuffer buff = new StringBuffer();
		buff.append("CREATE TABLE if not exists ").append(TABLE_CHAT_OTHER)
	...
	private Object[] getValue(PrivateChatMessagesEntity entity) {
		Object[] obj = new Object[] { entity.getUserId(),
				...
				entity.getMessageContent(), System.currentTimeMillis() + "",
	 ...
	 * @param entity
	 */
	public void saveChatOther(PrivateChatMessagesEntity entity) {
		SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
		try {
			db.beginTransaction();// 开始事务
			// 执行数据库
			db.execSQL(insertChatOther(), getValue(entity));
			db.setTransactionSuccessful();// 标记事务成功.
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 * @return
	 */
	public ArrayList<PrivateChatMessagesEntity> queryOtherChatRecord(
			String position, String size) {
		String sql = "SELECT * FROM " + TABLE_CHAT_OTHER
				+ " WHERE userId = ? order by messageDate desc limit ?,?";
		SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
		ArrayList<PrivateChatMessagesEntity> list = new ArrayList<PrivateChatMessagesEntity>();
		try {
			db.beginTransaction();// 开始事务
			...
			Cursor cursor = db.rawQuery(sql,
					new String[] { UserInfoBean.getUserInfo(context).getUid(),
							position, size });
	 ...
	 * @param messageDate
	 */
	public boolean isPrivateMsgExist(String sendedId, String receiveId,
			String messageDate) {
		boolean b;
		String sql = "SELECT * FROM "
				+ TABLE_CHAT_PERSONAL
	...
	public boolean deleteOtherChat() {
		...
		try {
			...
			// 执行数据库
			int delete = db
					.delete(TABLE_CHAT_OTHER, "userId = ?",
							new String[] { UserInfoBean.getUserInfo(context)
									.getUid() });
			b = true;
			db.setTransactionSuccessful();// 标记事务成功.
		} catch (Exception e) {
	...
	private Object[] getValue(GroupChatMessages entity) {
		Object[] obj = new Object[] { entity.getUserId(), entity.getGroupId(),
				...
				System.currentTimeMillis() + "", entity.getMessageType(),
	...
	private String insertNoticeContacts() {
		StringBuffer sb = new StringBuffer();
		sb.append("INSERT OR REPLACE INTO ")
				.append(TABLE_NOTICE_CONTACTS)
				.append(" (Gid,N_newMsgRemind,showNikeName,userId,isMember,backgroundPic)")
				.append("VALUES (?,?,?,?,?,?)");
	...
	private void mapToNoticeContacts(NoticeContacts notice, Cursor cursor) {
		if (cursor != null && cursor.getCount() > 0) {
			...
			notice.setBackgroundPic(cursor.getString(cursor
					.getColumnIndex("backgroundPic")));
		}
	}
	...
	private Object[] getValue(RecentlyContacts recently) {
		Object[] obj = new Object[] { recently.getUserId(),
				...
				recently.getMessageContent(), System.currentTimeMillis() + "",
